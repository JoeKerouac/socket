...
public class Starter {
    ...
    static void useLocal() {
        ...
        LocalMessageCenter publishCenter = new LocalMessageCenter();
        enviroment.put(Const.REGISTRY, registry);
        enviroment.put(Const.MSG_CENTER, publishCenter);
    ...
    static void useNet() {
        ...
        RedisBaseConfig redisBaseConfig = RedisClusterManagerFactory.buildRedisConfig(redisHost,
        ...
        clusterConfig = ClusterConfig.builder().registry(ZKRegistry.class.getName())
            .publishCenter(RedisMessageCenter.class.getName()).build();
    }
    ...
    static void startBalance() {
        try {
            com.joe.easysocket.server.balance.Config config = com.joe.easysocket.server.balance.Config
                .builder().connectorManager(TCPConnectorManager.class.getName())
                .clusterConfig(clusterConfig).port(10051).host(host).environment(enviroment)
                .build();
    ...
    static void startBackserver(BeanContainer beanContainer) {
        try {
            Config config = Config.builder().beanContainer(beanContainer)
                .clusterConfig(clusterConfig).host(host).name("后端" + System.currentTimeMillis())
                .environment(enviroment).build();
